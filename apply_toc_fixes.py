...
fixes = {
    ...
    # We'll map them to where they actually exist, but this might need manual review
...
for old_path, new_path in fixes.items():
    # Create a regex pattern that matches the old path in markdown links
    # Need to escape special regex characters
    escaped_old = re.escape(old_path)
    pattern = f'(\[{re.escape("[" + "]" + "(") if False else ""}\([^)]+\)\]\()({escaped_old})(\))'
    
    # Actually, simpler approach - just replace the path in markdown links
    # Pattern: [text](/6_1/old-path)
...
# Also handle the duplicate entries for sync_logs and tags in ITAM section
# There are two ITAM sections, so we need to fix both
...
for old_path, new_path in fixes.items():
    if old_path in content:
        print(f"WARNING: {old_path} still found in content")
    if new_path in content:
        print(f"[OK] {new_path} found in content")
