...
4. Configure escalation rules:
 ...
 1. [First column]{.Strong}: Choose whether the escalation fires **before** or **after** the specified delay.

 2. [Second column]{.Strong}: Enter the delay amount (hours/minutes) before notifications are sent.

 3. [Third column]{.Strong}: Click [Add]{.Strong} (for a new recipient list) or **Update** (to edit an existing one), then select users, roles, groups, or departments to alert.

For each rule, specify when it triggers, how long to wait, and who gets notified:

 -------------------------------------------- --------------------------------------------- ---------------------------------------------------------------------------------------------------------------------
 [When]{.Strong} [Delay]{.Strong} [Notification Recipients]{.Strong}
